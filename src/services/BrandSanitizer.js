...
export class BrandSanitizer {
  static clean(ragContext) {
    if (!ragContext || !ragContext.data) return null;

    // 1. Aplanar todo el texto del RAG para análisis
    // Si ragContext.data es array (chunks), los unimos. Si es objeto, stringify.
    const rawText = Array.isArray(ragContext.data)
      ? ragContext.data.map((d) => d.content_text || "").join(" ")
      : JSON.stringify(ragContext.data);
