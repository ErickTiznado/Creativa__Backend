...
class ValidationService {
  ...
  validateRequest(body, user) {
    ...
    if (style) {
      if (!PROMPT_CONFIG.STYLES.includes(style)) {
        throw new ValidationError(
          `Estilo '${style}' no válido. Permitidos: ${PROMPT_CONFIG.STYLES.join(", ")}`,
        );
      }
  ...
  validateImageGenerationRequest(body) {
    ...
    if (!prompt || typeof prompt !== "string" || prompt.trim().length === 0) {
      throw new ValidationError(
        "El campo 'prompt' es obligatorio para generar imágenes.",
      );
    }
    ...
    if (!allowedRatios.includes(validRatio)) {
      throw new ValidationError(
        `Aspect Ratio '${aspectRatio}' no soportado. Permitidos: ${allowedRatios.join(", ")}`,
      );
    }
    ...
    if (
      isNaN(validCount) ||
      validCount < MIN_SAMPLE_COUNT ||
      validCount > MAX_SAMPLE_COUNT
    ) {
      throw new ValidationError(
        `Sample Count inválido. Debe ser entre ${MIN_SAMPLE_COUNT} y ${MAX_SAMPLE_COUNT}.`,
      );
    }

    // 4. Validad Campaign ID
    let validCampaignId = campaignId
      ? this.sanitizeInput(campaignId)
      : undefined;

    // 5. Validar Estilo (Opcional)
    let validatedStyle = "corporate"; // Default
    if (style) {
      // Normalización para UX: "3d render" -> "3d-render"
      const normalizedStyle = style.trim().toLowerCase().replace(/\s+/g, "-");

      if (!PROMPT_CONFIG.STYLES.includes(normalizedStyle)) {
        // Opción: lanzar error o fallback. Lanzamos error para consistencia.
        throw new ValidationError(
          `Estilo '${style}' no válido. Permitidos: ${PROMPT_CONFIG.STYLES.join(", ")}`,
        );
      }
      validatedStyle = normalizedStyle;
    }
  ...
  validateBrief(brief) {
    ...
    if (brief.length < PROMPT_CONFIG.VALIDATION.MIN_BRIEF_LENGTH) {
      throw new ValidationError(
        `El brief es muy corto (${brief.length} chars). Mínimo requerido: ${PROMPT_CONFIG.VALIDATION.MIN_BRIEF_LENGTH} caracteres.`,
      );
    }

    if (brief.length > PROMPT_CONFIG.VALIDATION.MAX_BRIEF_LENGTH) {
      throw new ValidationError(
        `El brief excede el límite de ${PROMPT_CONFIG.VALIDATION.MAX_BRIEF_LENGTH} caracteres.`,
      );
    }
  }
  ...
  validateDimensions(dimensions) {
    if (
      ...
      !this.dimensionsPattern.matches(dimensions)
    ) {
      throw new ValidationError(
        "Formato de dimensiones inválido. Use formato ANCHOxALTO (ej: 1920x1080)",
      );
    }
  ...
  validateVariations(variations) {
    ...
    if (num < MIN || num > MAX) {
      throw new ValidationError(
        `Las variaciones deben estar entre ${MIN} y ${MAX}.`,
      );
    }
  }
