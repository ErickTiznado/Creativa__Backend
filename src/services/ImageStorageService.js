...
class ImageStorageService {
    ...
    async approvedAsset(assetId, userId) {
        ...
        const assetThumbnail = urlPrefixPattern.replace(asset.img_url.thumbnail, '')
        const newUrl = assetUrl.replace(save_location.temp, save_location.approved)
        const newThumbnail = assetThumbnail.replace(save_location.temp, save_location.approved)


        try {


            const approveAssets = await bucket.file(assetUrl).copy(bucket.file(newUrl));
            const approveAssetsThumbnail = await bucket.file(assetThumbnail).copy(bucket.file(newThumbnail));
    ...
    async fetchAssetsAsGeminiParts(assetIds) {
        ...
        for (const id of assetIds) {
            ...
            try {
                ...
                console.warn(`[ImageStorage] Error descargando asset ${id}: ${err.message}`);
            }
        }
